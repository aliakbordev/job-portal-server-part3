...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.ltyf59a.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    // create jwt token and store it in httpOnly cookie and clear
    app.post("/jwt", (req, res) => {
      const email = req.body.email;
      const token = jwt.sign({ email }, "secrete-key", { expiresIn: "1h", });
      res
        .cookie("token", token, 
            {
          httpOnly: true,
    ...
    // token verify function or middleware
